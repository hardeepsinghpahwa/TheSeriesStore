...
class SubCategories : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        firebaseRecyclerAdapter = object : FirebaseRecyclerAdapter<itemdetails, ItemViewHolder>(options) {
            override fun onBindViewHolder(holder: ItemViewHolder, position: Int, model: itemdetails) {
...
//                holder.category.setText(model.getCategory());
            ...
            override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemViewHolder {
                val view1 = LayoutInflater.from(parent.context).inflate(R.layout.productitem, parent, false)
        ...
        FirebaseDatabase.getInstance().reference.child("Items").addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (dataSnapshot in snapshot.children) {
                    if (!its!!.contains(dataSnapshot.child("subcategory").getValue(String::class.java))) {
    ...
    inner class SubAdapter(var items: ArrayList<String>) : RecyclerView.Adapter<SubCategoryViewHolder>() {
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SubCategoryViewHolder {
            val view1 = LayoutInflater.from(parent.context).inflate(R.layout.listitem, parent, false)
        ...
        override fun onBindViewHolder(holder: SubCategoryViewHolder, position: Int) {
            ...
            if (home.cat == items[position]) {
                ...
                home.cat = ""
                val query = FirebaseDatabase.getInstance().reference.child("Items").orderByChild("subcategory").equalTo(holder.name.text.toString())
                val options = FirebaseRecyclerOptions.Builder<itemdetails>()
                        .setQuery(query) { snapshot -> itemdetails(snapshot.child("image").getValue(String::class.java), snapshot.child("name").getValue(String::class.java), snapshot.child("rating").getValue(String::class.java), snapshot.child("category").getValue(String::class.java), snapshot.child("price").getValue(String::class.java), snapshot.key, snapshot.child("product").getValue(String::class.java)) }.build()
                val handler = Handler(Looper.getMainLooper())
                    ...
                    if (position == i) {
                        ...
                        if (textView != null) {
                            ...
                            if (textView.text.toString() == "All") {
                                ...
                                val query = FirebaseDatabase.getInstance().reference.child("Items").orderByChild("subcategory").equalTo(textView.text.toString())
                                val options = FirebaseRecyclerOptions.Builder<itemdetails>()
                                        .setQuery(query) { snapshot -> itemdetails(snapshot.child("image").getValue(String::class.java), snapshot.child("name").getValue(String::class.java), snapshot.child("rating").getValue(String::class.java), snapshot.child("category").getValue(String::class.java), snapshot.child("price").getValue(String::class.java), snapshot.key, snapshot.child("product").getValue(String::class.java)) }.build()
                                val handler = Handler(Looper.getMainLooper())
                                handler.postDelayed({ firebaseRecyclerAdapter!!.updateOptions(options) }, 100)

            ...
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    for (dataSnapshot in snapshot.children) {
                        if (!its.contains(
                                dataSnapshot.child("subcategory").getValue(String::class.java)
                            )
                        ) {
                            its.add(
                                dataSnapshot.child("subcategory").getValue(String::class.java)!!
        ...
        override fun onBindViewHolder(holder: SubCategoryViewHolder, position: Int) {
            ...
            if (home.cat == items[position]) {
                ...
                val query = FirebaseDatabase.getInstance().reference.child("Items")
                    .orderByChild("subcategory").equalTo(holder.name.text.toString())
                val options = FirebaseRecyclerOptions.Builder<itemdetails>()
                    .setQuery(query) { snapshot ->
                        itemdetails(
                            snapshot.child("image").getValue(String::class.java),
                    ...
                    if (position == i) {
                        ...
                        if (textView != null) {
                            ...
                            if (textView.text.toString() == "All") {
                                ...
                                val query = FirebaseDatabase.getInstance().reference.child("Items")
                                    .orderByChild("subcategory").equalTo(textView.text.toString())
                                val options = FirebaseRecyclerOptions.Builder<itemdetails>()
                                    .setQuery(query) { snapshot ->
                                        itemdetails(
                                            snapshot.child("image").getValue(String::class.java),
